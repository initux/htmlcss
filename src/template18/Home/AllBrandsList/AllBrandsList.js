...
const AllBrandsList = () => {
    const { brands, handleAllBrands, handleBrands, getStarting } = useAuth();
            ...
            <Container>
                    <Link to="/products">
                        <div className="tmp15-brands-list">
                            {
                                brands?.slice(0).reverse().map(brand => <div key={brand.name} className="tmp15-brands-list-inner" onClick={handleBrands}>
