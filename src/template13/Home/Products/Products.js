    ...
    return (
        <div>
            <style type="text/css">
                {
                  `
                    .tmp13-all-products-btn:hover, .tmp13-products-dropdown-content span:hover{
            ...
            <div className="tmp13-all-products-container">
                <div className="tmp13-search-container">
                                ...
                                <div style={{display: 'flex', alignItems: 'center', justifyContent: 'flex-end', flexWrap: 'wrap', marginBottom: '40px'}}>
                                    {
                                        uniqueCategoryLogo?.map(unique => <div key={unique.name}>
                                            <div className="tmp13-products-dropdown-container w-100">
                                                <Link to={`/${unique.name}`}>
                                                    <div onClick={handleCategory} style={{display: 'flex', alignItems: 'center', flexDirection: 'column', width: '120px', boxShadow: 'rgba(99, 99, 99, 0.2) 0px 2px 8px 0px', borderRadius: '5px', margin: '8px', padding: '10px 10px 0px', cursor: 'pointer'}}>
                                                        ...
                                                        <button className='tmp13-all-products-btn' style={{color: `${selectedCategory === unique.name ? getStarting?.primaryColor :'#4b5563'}`}} >{unique.name}</button>
                                                    </div>
                                                </Link>
                                                <div className="tmp13-products-dropdown-content" style={{height: unique?.subCategories?.length > 0 ? '650px' : '0px', padding: unique?.subCategories?.length > 0 ? '30px' : '0px'}}>
                        ...
                        <div className="d-flex flex-column justify-content-between">
                            <div>
                                {   products?.length === 0 ?
                                    ...
                                    :
                                    <div className="tmp13-all-products-inner">
                                        {
