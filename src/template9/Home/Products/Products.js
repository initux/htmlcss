...
const Products = () => {
    ...
    return (
        <div className="tmp9-products-container">
            <Container>
                <div className="tmp9-products-title text-center" onClick={handleClearWithoutCategories}>
                    {/* <span>Cosmetics</span> */}
                    ...
                    {   uniqueCategory?.length === 1 ?
                        ...
                        :
                        uniqueCategory.map(unique => <button onClick={handleCategory} className='tmp9-all-product-btn' style={{color: `${selectedCategory === unique ? '#fff' : '#666666'}`, backgroundColor: `${selectedCategory === unique ? getStarting?.primaryColor : '#fff'}`}} key={unique}>{unique}</button>)
                    }
                ...
                
                {
                     ...
                     :
                    <div className="tmp9-products-inner">
                        {
                            displayProducts?.slice(0, 20).map( product => <ProductsSummery product={product} key={product._id}></ProductsSummery>)
                        }
                    </div>
                }
                <div className="text-center my-5">
                    <Link to="/products" onClick={handleClearAllProductsPage}>
                        <button style={{ background: `${getStarting?.primaryColor}`, color: '#fff', border: 'none', padding: '10px 30px', borderRadius: '3px' }}>All Products</button>
                    </Link>
                </div>
            </Container>
        </div>
