            ...
            <Container>
                {
                    ...
                    <div>
                        {
                            newCtProducts?.map(category => <div key={category.category_id} className="tem1-categories-container">
                                <div style={{paddingBottom: '10px'}}>
                                    <div className="d-flex align-item-center justify-content-between" style={{borderBottom: '1px solid var(--color9)'}}>
                                        <h3 style={{marginTop: '15px', marginBottom: '0px'}}>{category.name}</h3>
                                        <Link to="/products" onClick={handleClearWithoutCategories}>
                                            <button className="tem1-categories-products-btn" style={{backgroundColor: `${getStarting?.primaryColor}`}}>
                                                <span>More All <div onClick={handleCategory} className="tem1-categories-products-btn-inner">{category.name}</div></span> 
                                            </button>
                                        </Link>
                                    </div>
                                </div>
                                <div className="tem1-categories-products">
                                    {
