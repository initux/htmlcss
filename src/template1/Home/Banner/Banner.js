...
const Banner = () => {
    const { getStarting, additionalImages, categories, handleCategory, selectedCategory, setHeaderSearchText} = useAuth();
    ...
    return (
        <Container>
             <style type="text/css">
                {
                  `.tmp1-banner-slider .slick-prev, .tmp1-banner-slider .slick-next{
                        background-color: ${getStarting?.primaryColor} !important;
                   }
                   .tmp1-banner-slider .slick-dots li.slick-active button:before{
                        color: ${getStarting?.primaryColor} !important;
                        font-size: 14px !important;
                    }
                   .tmp1-banner-dropdown-container span:hover{
                        color: ${getStarting?.primaryColor} !important;
                    }
                   `
                }
            </style>
            <div className="tmp1-banner-container">
                <div className="tmp1-banner-categories">
                    <div className="tmp1-banner-menu-header" style={{backgroundColor: `${getStarting?.primaryColor}`, border: `1px solid ${getStarting?.primaryColor}`}}>
                                    ...
                                    {   
                                        ...
                                        <>
                                            {Array.from(Array(9)).map((_, i) => (
                                                <div className="tmp1-banner-menu-categories" key={i}><Placeholder animation="glow"><Placeholder xs={8} /></Placeholder></div>
                                            ))}
                                        </>
                                        :
                                        categories?.slice().reverse().map(unique => <div key={unique.name} className="tmp1-banner-menu-categories d-flex align-items-center w-100">
                                            ...
                                            <div className="tmp1-banner-dropdown-container w-100">
                                                ...
                                                <div className="tmp1-banner-dropdown-content" style={{height: unique?.subCategories?.length > 0 ? '450px' : '0px', padding: unique?.subCategories?.length > 0 ? '30px' : '0px'}}>
                ...
                {   
                    ...
                    <div className="tmp1-banner-slider">
